...
class TestLoadConfigPlatformValidation:
    ...
    def test_platforms_omitted_is_valid(self, tmp_path):
        d = self._write_config(tmp_path, [
            {"name": "cpu", "dockerfile": "Dockerfile"}
        ])
        result = load_config(d)
...
class TestGenerateForIndexRaceCondition:
    def test_failing_group_does_not_delete_temp_dir_early(self, monkeypatch):
        ...
        def mock_run_pip_command(cmd, timeout, phase_name, **kwargs):
            ...
            call_count["slow"] += 1
            if captured_tmpdir:
                assert Path(captured_tmpdir[0]).exists(), (
                    "Temp dir was deleted while other groups still running"
                )
            return subprocess.CompletedProcess(
                args=cmd, returncode=0, stdout="", stderr=""
            )
        ...
        monkeypatch.setattr(
            _mod, "collect_hashes_from_download_dir", lambda *a: {}
        )
        ...
        result = generate_for_index_fn(
            ...
            out_path=Path("/tmp/test-out.txt"),
            dry_run=False,
            download_timeout=60,
        )
        assert result == 1

    def test_all_failures_reported(self, monkeypatch, capsys):
        generate_for_index_fn = _mod.generate_for_index
    ...
    def test_successful_groups_still_collected_on_partial_failure(self, monkeypatch):
        generate_for_index_fn = _mod.generate_for_index
        collected_dirs = []
        ...
        monkeypatch.setattr(_mod, "collect_hashes_from_download_dir", mock_collect_hashes)
...
class TestDryRun:
    def test_dry_run_uses_resolved_platform_tags(self, capsys, monkeypatch):
        generate_for_index_fn = _mod.generate_for_index
        monkeypatch.setattr(
            _mod, "_pip_supports_report", lambda: (True, "")
        )
        result = generate_for_index_fn(
            index_url="https://example.com/simple",
            root_names=["mlserver"],
            platform_groups=[PLATFORM_TAGS["x86_64"]],
            out_path=Path("/tmp/dry-out.txt"),
            dry_run=True,
            download_timeout=480,
        )
        assert result == 0
    ...
    def test_dry_run_respects_variant_platforms(self, capsys, monkeypatch):
        generate_for_index_fn = _mod.generate_for_index
        monkeypatch.setattr(
            _mod, "_pip_supports_report", lambda: (True, "")
        )
    ...
    def test_dry_run_returns_zero(self, capsys, monkeypatch):
        generate_for_index_fn = _mod.generate_for_index
        monkeypatch.setattr(
            _mod, "_pip_supports_report", lambda: (True, "")
        )
