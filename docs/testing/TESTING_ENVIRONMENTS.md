...
| `runtimes-cuda` | mlserver-onnx[cuda] + nvidia-cublas-cu12, nvidia-cudnn-cu12, etc. | CUDA-accelerated ONNX runtime. `mlserver-onnx[cuda]` provides `onnxruntime-gpu`; NVIDIA pip libraries are dev-only (production uses system RPMs). Requires GPU hardware. Run via `make test-cuda` or `tox -c ./runtimes/onnx -e cuda`. `@pytest.mark.cuda` tests auto-skip on non-GPU machines. Included automatically by `make install-dev-cuda`. |
