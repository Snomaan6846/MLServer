...
def collect_hashes_from_download_dir(
    ...
    """Collect hashes by parsing downloaded artifact filenames."""
...
def _pip_supports_report(min_major: int = 22, min_minor: int = 2) -> tuple[bool, str]:
    ...
    if (major, minor) < (min_major, min_minor):
        return (
            False,
            f"Detected pip {major}.{minor}; this script requires pip >= {min_major}.{min_minor} "
            "because it uses `pip --report`.",
        )
    return True, output
...
def run_pip_command(
    ...
    if return_code != 0:
        ...
        req_note = (
            f" Suspected requirement: {failed_req}."
            if failed_req
            else ""
        )
        raise RuntimeError(
...
def main() -> int:
    ...
    if args.index_url:
        print(f"Output -> {out_path} (index: {args.index_url})")
    else:
