...
def _kill_registered_procs(
    procs: list[subprocess.Popen], lock: threading.Lock
) -> None:
        ...
        try:
            ...
            if failures:
                for group, err in failures:
                    print(
                        f"  Phase 2 failed for {group}: {err}", file=sys.stderr
                    )
                return 1
        finally:
...
def main() -> int:
    ...
    try:
        ...
        return 1
    print(
        f"Platforms ({platform_source}): "
        f"{[g[0] for g in platform_groups]}"
    )
