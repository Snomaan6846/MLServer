...
def sanitize_log_line(line: str) -> str:
    """Redact index URL occurrences in pip output lines."""
...
def format_command_for_log(cmd: list[object]) -> str:
    """Render command for logs with sensitive --index-url values redacted."""
    rendered: list[str] = []
    i = 0
    while i < len(cmd):
        token = str(cmd[i])
...
def run_pip_command(
    ...
    """Run pip command with live output, retries, and rich diagnostics."""
